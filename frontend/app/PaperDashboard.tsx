...
export default function PaperDashboard({
  ...
  const loadStrategies = useCallback(async () => {
    try {
      const res = await fetch("/quant/api/paper?view=strategies", { cache: "no-store" });
  ...
  const loadState = useCallback(async (strategy: string) => {
    try {
      const [sRes, eRes] = await Promise.all([
        fetch(`/quant/api/paper?view=state&strategy=${strategy}`, { cache: "no-store" }),
        fetch(`/quant/api/paper?view=equity&strategy=${strategy}`, { cache: "no-store" }),
      ]);
    ...
    async (symbol: string, trade: TradeOverlay) => {
      ...
      try {
        const res = await fetch(
          `/quant/api/paper?view=klines&strategy=${active}&symbol=${symbol}&granularity=1H&limit=200`,
          { cache: "no-store" }
        );
        const json = await res.json();
