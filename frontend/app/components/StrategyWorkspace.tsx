...
import { loadClosedTrades, loadKlines, loadStrategyState, formatTime, money, percentage, shortSymbol } from "../lib/data";
...
function PaperTable({ strategy, positions, closed, refreshToken, onSelect, selectedId }: { strategy: StrategyKey; positions: Position[]; closed: Position[]; refreshToken?: string; onSelect: (position: Position) => void; selectedId?: number }) {
  const pageSize = 10;
  const [closedPage, setClosedPage] = useState(1);
  const [closedRows, setClosedRows] = useState<Position[]>(closed.slice(0, pageSize));
  const [closedTotal, setClosedTotal] = useState(closed.length);
  const [closedLoading, setClosedLoading] = useState(false);
  const [closedError, setClosedError] = useState("");
  const pageCount = Math.max(1, Math.ceil(closedTotal / pageSize));

  useEffect(() => {
    let alive = true;
    setClosedLoading(true);
    loadClosedTrades(strategy, pageSize, (closedPage - 1) * pageSize)
      .then((result) => {
        if (!alive) return;
        setClosedRows(result.closed || []);
        setClosedTotal(Number(result.total || 0));
        setClosedError("");
      })
      .catch((reason) => { if (alive) setClosedError(reason?.message || "已平仓记录读取失败"); })
      .finally(() => { if (alive) setClosedLoading(false); });
    return () => { alive = false; };
  }, [strategy, closedPage, refreshToken, positions.length, closed.length]);

  useEffect(() => {
    if (closedPage > pageCount) setClosedPage(pageCount);
  }, [closedPage, pageCount]);

  const renderRow = (position: Position) => <tr key={position.id} className={`clickable-row ${selectedId === position.id ? "paper-row-selected" : ""}`} tabIndex={0} role="button" aria-label={`查看${positionLabel(position)}`} onClick={() => onSelect(position)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(position); } }}><td><strong>{shortSymbol(position.symbol)}/USDT</strong><small>{position.tag}</small></td><td><span className={`direction-pill ${position.direction}`}>{position.direction === "long" ? "LONG" : "SHORT"}</span></td><td>{money(position.entry_price, position.entry_price < 10 ? 4 : 2)}</td><td>{money(position.stop_price, position.stop_price < 10 ? 4 : 2)} / {money(position.target_price, position.target_price < 10 ? 4 : 2)}</td><td className={(position.floatPnlUsd ?? position.pnl_usd ?? 0) >= 0 ? "positive" : "negative"}>{money(position.floatPnlUsd ?? position.pnl_usd ?? 0)}<small>{position.floatPnlPct != null ? ` ${percentage(position.floatPnlPct)}` : position.pnl_pct != null ? ` ${percentage(position.pnl_pct)}` : ""}</small></td><td><span className={`status-chip ${position.status === "open" ? "live" : "closed"}`}>{position.status === "open" ? "持仓中" : position.exit_reason || "已平仓"}</span></td></tr>;
  const table = (rows: Position[], empty: string) => <div className="table-scroll"><table className="data-table compact"><thead><tr><th>合约</th><th>方向</th><th>入场</th><th>止损 / 止盈</th><th>盈亏</th><th>状态</th></tr></thead><tbody>{rows.map(renderRow)}</tbody></table>{!rows.length ? <div className="empty-state">{empty}</div> : null}</div>;

  return <div className="paper-table-wrap"><div className="subsection-head"><div><span className="table-kicker">AUTOMATED PAPER LEDGER</span><h3>模拟仓记录</h3></div><span className="muted">后台自动开仓 · 盯仓 · 平仓</span></div><div className="paper-table-note">这里按交易状态分开：上面是当前正在持仓的单子，下面是已经完成的平仓单。点击任意记录，可查看入场、止损、止盈和 K 线位置。</div><section className="paper-ledger-section"><div className="ledger-section-head"><div><strong>当前持仓</strong><span>正在被策略管理，浮动盈亏会随行情更新</span></div><span className="ledger-count live">{positions.length} 笔</span></div>{table(positions, "当前没有持仓，策略命中后会由后台自动开仓。")}</section><section className="paper-ledger-section closed-ledger"><div className="ledger-section-head"><div><strong>已平仓记录</strong><span>按平仓时间倒序，完整记录可翻页查看</span></div><span className="ledger-count">共 {closedTotal} 笔</span></div>{closedLoading ? <div className="ledger-loading">正在读取平仓记录…</div> : table(closedRows, "当前还没有已平仓记录。")}{closedError ? <div className="ledger-error">{closedError}</div> : null}{closedTotal > pageSize ? <div className="ledger-pagination"><span>第 {closedPage} / {pageCount} 页 · 每页 {pageSize} 笔</span><div><button type="button" className="outline-button" disabled={closedPage <= 1 || closedLoading} onClick={() => setClosedPage((page) => page - 1)}>上一页</button><button type="button" className="outline-button" disabled={closedPage >= pageCount || closedLoading} onClick={() => setClosedPage((page) => page + 1)}>下一页</button></div></div> : null}</section></div>;
}
...
function CandidateInspector({ candidate, position, candles, loading, error, mode }: { candidate: Candidate; position?: Position; candles: Kline[]; loading: boolean; error?: string; mode: "anomaly" | "dualma" | "box-breakout" }) {
  const entry = position?.entry_price || candidate.lastPrice;
  const stop = position?.stop_price || candidate.lastPrice * (candidate.direction === "long" ? (mode === "box-breakout" ? 0.95 : 0.97) : 1.03);
  const target = position?.target_price || candidate.lastPrice * (candidate.direction === "long" ? (mode === "box-breakout" ? 1.1 : 1.06) : 0.94);
  ...
  const overlay: TradeOverlay = { direction: candidate.direction, entry, stop, target, openTime: position?.open_time ? Math.floor(position.open_time / 1000) : undefined, entryMarkerTime: markerTime, entryLabel: position ? "实际开仓" : mode === "dualma" ? "信号/计划" : mode === "box-breakout" ? "突破计划" : "计划入场", entryLineTitle: position ? "实际入场" : "计划入场" };
  return <div className="candidate-inspector-chart"><div className="candidate-chart-head"><div><strong>{position ? "实际 Paper 开仓位置" : "策略计划入场位置"}</strong><span>{mode === "dualma" ? "标记在已收盘交叉 K 线" : mode === "box-breakout" ? "标记在已收盘 30m 突破 K 线" : "标记在最近一根可用 K 线"}</span></div><span className="chart-key"><i className="chart-key-entry" />{position ? "实际入场" : "计划入场"}</span></div>{loading ? <div className="chart-loading">正在读取 {shortSymbol(candidate.symbol)} K 线…</div> : candles.length ? <TradeChart candles={candles} trade={overlay} height={270} /> : <div className="chart-loading">K 线暂时不可用，但候选数据已保留。{error ? `（${error}）` : ""}</div>}</div>;
}
...
export default function StrategyWorkspace({ mode }: { mode: "anomaly" | "dualma" | "box-breakout" }) {
  const strategy: StrategyKey = mode === "anomaly" ? "anomaly-binance" : mode === "dualma" ? "dualma4h-binance" : "box-breakout30m-binance";
  const timeframe = mode === "anomaly" ? "1H" : mode === "dualma" ? "4H" : "30m";
  const boxMode = mode === "box-breakout";
  ...
  const candidates = mode === "anomaly" || boxMode ? allCandidates : liveCandidates;
  const hasLiveAnomalyScan = mode === "anomaly" && Boolean(state?.latestScan?.scannedAt);
  const displayed = useMemo(() => {
    const filtered = candidates.filter((candidate) => filter === "全部" || (filter === "多头" ? candidate.direction === "long" : filter === "空头" ? candidate.direction === "short" : statusFor(candidate, positions) === "Paper 已开仓"));
    const valueFor = (candidate: Candidate) => {
      if (sort === "oiChangePct") return candidate.oiChangePct ?? -Infinity;
      if (sort === "change24hPct") return candidate.change24hPct ?? -Infinity;
      if (sort === "volumeUsd") return candidate.volumeUsd ?? -Infinity;
      if (sort === "fundingRate") return candidate.fundingRate ?? -Infinity;
      return sort === "volumeRank" || sort === "volume" ? -(candidate.volumeRank || 99) : candidate.score;
    };
    return filtered.sort((a, b) => (valueFor(b) - valueFor(a)) * (sortDirection === "desc" ? 1 : -1));
  }, [candidates, filter, positions, sort, sortDirection]);
  ...
  useEffect(() => {
    ...
    setCandidateCandles([]);
    loadKlines(strategy, selected.symbol, timeframe).then((items) => { if (alive) setCandidateCandles(items); }).catch((reason) => { if (alive) { setCandidateCandles([]); setCandidateKlineError(reason?.message || "K 线读取失败"); } }).finally(() => { if (alive) setCandidateLoading(false); });
    return () => { alive = false; };
  }, [mode, selected, strategy, timeframe]);
  ...
  useEffect(() => {
    ...
    setPaperKlineError("");
    setPaperCandles([]);
    loadKlines(strategy, paperSelection.symbol, timeframe).then((items) => { if (alive) setPaperCandles(items); }).catch((reason) => { if (alive) { setPaperCandles([]); setPaperKlineError(reason?.message || "K 线读取失败"); } }).finally(() => { if (alive) setPaperLoading(false); });
    return () => { alive = false; };
  }, [mode, paperSelection, strategy, timeframe]);

  return <div className={`workspace-stack ${mode}-workspace`}>
    ...
    {mode === "anomaly" ? <div className="candidate-sort-controls" aria-label="候选排序控制"><span>候选排序</span><select aria-label="排序指标" value={sort} onChange={(event) => setSort(event.target.value)}><option value="score">评分</option><option value="oiChangePct">OI 变化</option><option value="change24hPct">价格变化</option><option value="volumeUsd">成交量</option><option value="fundingRate">资金费率</option></select><button type="button" className="sort-direction-button" aria-label={sortDirection === "desc" ? "当前从大到小，切换为从小到大" : "当前从小到大，切换为从大到小"} onClick={() => setSortDirection((direction) => direction === "desc" ? "asc" : "desc")}>{sortDirection === "desc" ? "↓ 从大到小" : "↑ 从小到大"}</button><small>只影响下方候选表，不改变策略开仓规则</small></div> : null}
    ...
    <PaperTable strategy={strategy} positions={positions} closed={closed} refreshToken={state?.latestScan?.scannedAt || ""} onSelect={setPaperSelection} selectedId={paperSelection?.id} />
