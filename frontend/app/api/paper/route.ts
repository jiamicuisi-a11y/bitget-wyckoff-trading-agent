 ...
 * 模拟盘数据代理。
 *
 * 真正跑模拟盘的是同机的「量化 worker」（多策略 Node 进程，默认 127.0.0.1:8800），
 * 它负责定时扫描全市场、按各策略自动模拟开/平仓、把数据写进 SQLite。本路由只是把
 * worker 的状态读出来转发给前端，避免浏览器直连 worker 带来的跨域/暴露问题。
 *
 * worker 地址用环境变量 QUANT_WORKER_URL 覆盖，默认 http://127.0.0.1:8800。
 *
 ...
 */
const WORKER = process.env.QUANT_WORKER_URL || "http://127.0.0.1:8800";
