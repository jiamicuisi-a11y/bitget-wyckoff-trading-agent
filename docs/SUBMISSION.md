...
把市场情报、A 档异动扫描、4H 双均线和 30 分钟箱体突破策略，编排成一条“事件感知 → 行情关联 → 策略判断 → 风险闸门 → 人工确认 → Paper 执行 → 审计”的可解释 Agent 工作流。

## Description

Strategy Copilot 是一个面向 Binance Futures 公开市场数据的交易研究 Agent。它继承此前 Bitget Hackathon 项目的 A 档异动扫描和双均线策略，并新增 30 分钟箱体突破策略：A 档综合 OI、价格、成交额、盘口和资金费率筛选机会；双均线在成交额 Top30 永续合约上计算 4H EMA(10/30) 交叉信号；箱体策略只在已收盘 K 线向上突破并放量时做多。新增的市场情报中心读取 Binance 中文站公开活动/公告，保留中文原文来源与时间，并只读关联公开行情和策略候选。
...
- 三套 Binance 策略注册：A 档异动扫描、4H 双均线、30 分钟箱体突破。
...
- [ ] 录制 2–3 分钟 Demo：总览 → 市场情报/活动规则 → A 档 → 双均线 → Agent 对话 → MCP Tool Trace → Paper / Risk Gate。
