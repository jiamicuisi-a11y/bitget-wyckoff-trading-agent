...
import { latestClosedSignal } from "./indicators.mjs";
...
function anomalyCandidates(ctx) {
  const { allCandidates } = scanTickers(ctx.tickers, ctx.prevOi, SCAN_CONFIG);
  ...
  return async function dualmaCandidates(ctx) {
    ...
    for (let i = 0; i < universe.length; i += BATCH) {
      ...
      const results = await Promise.allSettled(
        batch.map(async (sym) => {
          const candles = await ctx.fetchKlines(sym, granularity, klineLimit);
          return { sym, candles };
        })
      );
